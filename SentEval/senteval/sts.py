    ...
    def loadFile(self, fpath):
        ...
        with io.open(fpath, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                sick_data['y'].append(text[0])

        sick_data['y'] = [float(s) for s in sick_data['y']]
        self.samples += sick_data['X_A'] + sick_data["X_B"]
        return (sick_data['X_A'], sick_data["X_B"], sick_data['y'])

class JSTSEval(STSEval):
    def __init__(self, task_path, seed=1111):
        logging.debug('\n\n***** Transfer task : JSTS*****\n\n')
        self.seed = seed
        self.samples = []
        train = self.loadFile(os.path.join(task_path, 'JSTS-train.csv'))
        dev = self.loadFile(os.path.join(task_path, 'JSTS-dev.csv'))
        test = self.loadFile(os.path.join(task_path, 'JSTS-test.csv'))
        self.datasets = ['train', 'dev', 'test']
        self.data = {'train': train, 'dev': dev, 'test': test}
...
class JACSTSEval(STSEval):
    def __init__(self, task_path, seed=1111):
        logging.debug('\n\n***** Transfer task : JACSTS*****\n\n')
        self.seed = seed
        self.samples = []
        dev = self.loadFile(os.path.join(task_path, 'JAC-sts-dev.csv'))
        test = self.loadFile(os.path.join(task_path, 'JAC-sts-test.csv'))
